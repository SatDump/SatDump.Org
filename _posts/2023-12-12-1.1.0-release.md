...
you might want to check out the new autotrack function. Just tell SatDump the satellites you are interested in, and it will
...
JVital2013 added a Timestamp tool, that makes it really easy to add the starting time of your recording, so that you can get some great looking APT composites.
...
Soon after, support for the MERSI-RM, MWRI-RM and PMR-radar instruments was added to SatDump.
...
Fred Jansen managed to get a X-Band dump to Moscow, leading to the development of working MTVZA projections & global MTVZA coverage.
...
Thanks to SnazzLazz, who managed to get some RAW X-Band Data from GOES-16, Aang23 was able to develop a decoder for 
...
It's still work-in-progress and unfortunately you will need a very large dish to capture the high-rate signal containing most of the image data.
...
A small issue with a mutex has caused SatDump to crash when attempting to do two things at once.
