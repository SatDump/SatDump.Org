...
If you are especially lazy, you might want to check out the new autotrack function. Just tell SatDump the satellites yu are interested in, and it will automatically calculate the best passes, control your rotor and start decoding without you having to do anything!
...
The NOAA NO (precipitation likelyhood), MSA and MCIR enhancements found in WXtoIMG has been implemented into SatDump.
...
JVital2013 added a Timestamp tool, which makes it really easy to add the starting time of your recording, so that you can get some great looking APT composites.
...
Fred Jansen managed to get a Meteor X-Band dump to Moscow, which was found to contain recorded MTVZA data, leading to the development of working global MTVZA coverage and projections.
...
## Fixed METEOR-M timestamps

MSU-MR images from METEOR-M showed up in completely wrong locations due to the timestamps being parsed wrong. This is fixed now.
